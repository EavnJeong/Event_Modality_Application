...
class FTTrainer(pl.LightningModule):
    def __init__(self, model, args):
        ...
        if args.dataset == "N-imagenet":
            ...
            from data.imagenet_dataset import labels, text_inputs
            print("Dataset is ImageNet-1000")
            self.labels = labels
        elif args.dataset == "N-caltech":
            from data.caltech_dataset import labels, text_inputs
            print("Dataset is Caltech")
            self.labels = labels
        elif args.dataset == 'N-mnist':
            from data.mnist import labels, text_inputs
        ...
        for predi, predi5, true_label in zip(prediction_label, pred_list, label):
            if self.args['dataset'] == 'N-imagenet' or self.args['dataset'] == 'N-imagenet-1000':
                ...
                if self.labels[true_label] in predi5:
                    correct_5 += 1
            elif self.args['dataset'] == 'N-caltech' or 'N-mnist':
                if true_label == predi:
                    correct += 1
    ...
    def get_pred_loss(self, pred, label):
        ...
        if self.args['dataset'] == 'N-imagenet' or self.args['dataset'] == 'N-imagenet-1000':
            ...
            label_indices = torch.tensor(
                [list(self.labels.values()).index(true_label) 
                    for true_label in label]).to(pred.device)
    ...
    def training_step(self, data, idx):
        ...
        pred_loss = self.get_pred_loss(pred_k, label)
    ...
    def validation_step(self, data, idx):
        ...
        self.metric_logger.meters['pred_loss'].update(pred_loss.item(), n=batch_size)
    ...
    def validation_epoch_end(self, outputs):
        ...
        pred_loss = self.metric_logger.pred_loss.global_avg

        if not os.path.exists(self.args["save_path"]):
            os.makedirs(self.args["save_path"]) 

        print(f"Validation Accuracy: {acc1*100.0:.2f}%")
        torch.save(
            {
    ...
    def produce_log(
        ...
        pred_loss = self.all_gather(pred_loss).mean().item()
